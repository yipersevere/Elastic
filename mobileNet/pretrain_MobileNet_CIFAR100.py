...
def count_flops(model):
    """
    calculate flops value of the model
    """
    op_count = 0
...
def pretrain():
    ...
    imageStr = {
        ...
        "ax0_title": "MobileNet-alpha-0.75 test on CIFAR 100",
        "ax1_set_ylabel": "f1 score on CIFAR-100",
        "ax1_title": "f1 score MobileNet-alpha-0.75 test on CIFAR 100",
        "save_fig" : "MobileNet-alpha-0.75_CIFAR_100.pdf"
    }
   # ==============Done parameters============
    ...
    LOG("Pre-training CIFAR-100 on MobileNet, alpha = 0.75...", logFile)

    # 从keras中下载inceptionv3 model, 然后根据ElasticNN论文中的描述, 对inceptionV3模型修改
    # "3" 是指 三通道, 即 RGB
    # 论文中推荐的输入大小为 224 * 224，因此训练集最好使用同样的大小
